...
movie.get("/movie/:id", async (req, res) => {
  try {
    ...
    $(".card.border-0.mb-3.shadow").each((index, element) => {
      ...
      const actors =
        $(element)
          ...
          .trim() || null;
      const director =
        $(element).find(".row .mb-1:eq(2)").text().replace(/\n/g, "") || null;
      const country =
        $(element).find(".row .mb-1:eq(3)").text().replace(/\n/g, "") || null;
      const type =
        $(element).find(".row .mb-1:eq(4)").text().replace(/\n/g, "") || null;
      const releas =
        $(element).find(".row .mb-1:eq(5)").text().replace(/\n/g, "") || null;
      const imdbRating =
        $(element).find(".row .mb-1:eq(6)").text().replace(/\n/g, "") || null;
      const keywords =
        $(element)
          .find(".card-footer.border-0.tags")
          .text()
          .replace(/\n/g, "")
          .split("Keyword:")[1] || null;
      const bigbanner =
        $(element)
          .find(".col-12.col-lg-2.d-none.d-lg-block img")
          .attr("data-src") || null;
    ...
    const tep = $(".card.border-0.mb-3.shadow")
      .find(".row .mb-1:eq(3)")
      .text()
      .match(/Episode:\s*(\d+)/);
    ...
    $(".card.border-0.mb-3")
      ...
      .each((index, element) => {
        ...
        const id =
          $(element)
            .find(".card.border-0.shadow a")
            .attr("href")
            .split("https://123movies.org.mx/movie/")[1] || null;
